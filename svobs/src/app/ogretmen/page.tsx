...
function OgretmenNotlar({ dersId, sinifId, dersAd, supabase }: any) {
  ...
  async function kaydet() {
    if (!notBasligi.trim()) { alert('Not başlığı zorunlu!'); return }
    setKaydediyor(true)
    for (const o of ogrenciler) {
      const puan = notlar[o.id]
      if (!puan && puan !== '0') continue
      await supabase.from('notlar').insert({
        ogrenci_id: o.id, ders_id: dersId,
        baslik: notBasligi.trim(), puan: parseFloat(puan), tarih: notTarihi
      })
    }
    setNotlar({})
    setNotBasligi('')
  ...
  const grupluNotlar = gecmisNotlar.reduce((acc: any, n: any) => {
    const key = `${n.baslik}__${n.tarih}`
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setSekme('giris')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${sekme === 'giris' ? 'bg-green-700 text-white' : 'bg-white text-gray-600 border border-gray-300'}`}>
          Not Girişi
        </button>
        <button onClick={() => setSekme('gecmis')}
          className={`px-4 py-2 rounded-lg text-sm font-medium transition ${sekme === 'gecmis' ? 'bg-green-700 text-white' : 'bg-white text-gray-600 border border-gray-300'}`}>
          Geçmiş Notlar
        </button>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm">
          <div className="p-4 border-b border-gray-100 flex flex-wrap items-end gap-3">
            <div>
              ...
              <input value={notBasligi} onChange={(e) => setNotBasligi(e.target.value)}
                className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-600"
                placeholder="örn: 1. Sınav..." />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Tarih</label>
              <input type="date" value={notTarihi} onChange={(e) => setNotTarihi(e.target.value)}
                className="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-600" />
            </div>
            <button onClick={kaydet} disabled={kaydediyor}
              className="bg-green-700 text-white px-4 py-1.5 rounded-lg text-sm hover:bg-green-800 disabled:opacity-50">
